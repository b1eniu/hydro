🌊 hydro
Minimalistyczna aplikacja do monitorowania poziomu rzek w Polsce.

hydro to lekkie narzędzie webowe stworzone z myślą o szybkim i przejrzystym dostępie do danych hydrologicznych. Aplikacja pobiera dane w czasie rzeczywistym bezpośrednio z publicznego API IMGW, prezentując je w nowoczesnej, mobilnej formie.

✨ Funkcje
Dane w czasie rzeczywistym: Aktualne odczyty poziomu wód dla głównych rzek Polski (Wisła, Odra, Radomka i inne).

Inteligentna Lista: Podgląd 7 stacji wokół wybranego punktu (4 powyżej, 2 poniżej) dla lepszego kontekstu hydrologicznego.

Wizualizacja trendów: Interaktywne wykresy z możliwością zmiany skali czasowej (6h, 12h, 24h).

Bezpieczeństwo: Wyraźne oznaczenie stanów ostrzegawczych (pomarańczowy) i alarmowych (czerwony).

Personalizacja: Aplikacja zapamiętuje ostatnio przeglądaną stację oraz kierunek rzeki (źródło ➔ ujście).

Pogoda: Zintegrowany podgląd temperatury, opadów i wiatru dla wybranej lokalizacji.

🛠️ Technologia
Aplikacja została zbudowana jako Single Page Application (SPA) przy użyciu:

HTML5 / Tailwind CSS – dla responsywnego i nowoczesnego interfejsu.

JavaScript (ES6+) – obsługa logiki i dynamicznego renderowania.

Chart.js – generowanie wykresów poziomów wód.

IMGW-PIWB API – źródło oficjalnych danych hydrologicznych.

Open-Meteo API – dane pogodowe.

👤 Autor
b1eniu 📧 tomaszbieniek86@gmail.com# hydro
aplikacja do monitorowania stanu rzek
